    ...
    <div className="min-h-screen bg-dark py-12">
        ...
        <DialogContent className="bg-dark-secondary border-gray-700 text-white max-w-2xl">
          ...
          <form onSubmit={handleNFTSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {collections.length === 0 ? (
                  <div className="flex items-center h-10">
                    <span className="text-sm text-pi-muted">No collections available</span>
                    <Button type="button" variant="link" className="ml-2 h-auto p-0" onClick={handleCreateCollectionFromNFTDialog}>
                      Create Collection
                    </Button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    ...
                    <Button type="button" variant="outline" className="px-2" onClick={handleCreateCollectionFromNFTDialog}>
                      <Plus size={16} />
                    </Button>
                  </div>
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="border border-dashed border-gray-700 rounded-lg p-4 bg-dark">
                {imagePreview ? (
                  <div className="relative aspect-video max-h-[200px] overflow-hidden rounded">
                    <img 
                      src={imagePreview} 
                      alt="Preview" 
                  ...
                  <div className="flex flex-col items-center justify-center py-4">
                    ...
                    <p className="text-sm text-pi-muted mb-2">Drag and drop an image or click to browse</p>
                    <Input
                      id="image-upload"
                      type="file"
                      accept="image/*"
            ...
            {currentNFT.content_type !== 'image' && (
              <div className="space-y-2">
                <Label>Content File {currentNFT.content_type && `(${currentNFT.content_type})`}</Label>
                <div className="border border-dashed border-gray-700 rounded-lg p-4 bg-dark">
                  {contentFileInfo ? (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        {currentNFT.content_type === 'audio' && <Music size={18} className="mr-2 text-pi-muted" />}
                        {currentNFT.content_type === 'video' && <Film size={18} className="mr-2 text-pi-muted" />}
                        {currentNFT.content_type === 'book' && <Book size={18} className="mr-2 text-pi-muted" />}
                        <span className="text-sm">{contentFileInfo}</span>
                      </div>
                      <Button 
                        type="button" 
                        variant="destructive" 
                    ...
                    <div className="flex flex-col items-center justify-center py-4">
                      ...
                      <p className="text-sm text-pi-muted mb-2">
                        Upload your {currentNFT.content_type} file
                      </p>
                      <Input
                        id="content-upload"
                        type="file"
                        accept={
                          currentNFT.content_type === 'audio' ? 'audio/*' :
                          currentNFT.content_type === 'video' ? 'video/*' :
                          currentNFT.content_type === 'book' ? '.pdf,.epub,.mobi' :
                          '*/*'
                        }
                        onChange={handleContentFileChange}
                        className="hidden"
                      />
                      <Button type="button" variant="outline" onClick={() => document.getElementById('content-upload')?.click()}>
                        Select File
                      </Button>
                    </div>
                  )}
                ...
                <p className="text-xs text-pi-muted">
                  {currentNFT.content_type === 'audio' && 'Supported formats: MP3, WAV, OGG (max 30MB)'}
                  {currentNFT.content_type === 'video' && 'Supported formats: MP4, WebM (max 100MB)'}
                  {currentNFT.content_type === 'book' && 'Supported formats: PDF, EPUB (max 50MB)'}
                </p>
              </div>
            )}
      ...
      <Dialog open={isCollectionDialogOpen} onOpenChange={setIsCollectionDialogOpen}>
        <DialogContent className="bg-dark-secondary border-gray-700 text-white">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a collection to organize your NFTs
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="border border-dashed border-gray-700 rounded-lg p-4 bg-dark">
                {imagePreview ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-4">
                    <Package className="mb-2 text-pi-muted" size={32} />
                    <p className="text-sm text-pi-muted mb-2">Upload an image for your collection</p>
                    <Input
                      ...
                      className="hidden"
                    />
                    <Button type="button" variant="outline" onClick={() => document.getElementById('collection-image-upload')?.click()}>
                      Select Image
                    </Button>
                  </div>
                )}
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="bg-dark-secondary border-gray-700 text-white">
          ...
          <div className="mt-2 p-4 border border-red-400/20 bg-red-900/10 rounded-md">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-pi-muted">
                  Deleting an NFT will permanently remove it from your collection. If it has been minted on the blockchain, the token will still exist but will no longer be manageable from this platform.
                </p>
              </div>
            </div>
          </div>
          ...
          {nftToDelete && (
            <div className="flex items-center mt-2 border border-gray-700 rounded-md p-2">
              ...
              <div>
                ...
                <p className="text-xs text-pi-muted">
                  {nftToDelete.collection} • {nftToDelete.price} {nftToDelete.currency?.toUpperCase() || 'ETH'}
                </p>
              </div>
            </div>
          )}
